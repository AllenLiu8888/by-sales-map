import { useState } from "react";
import RegionList from "./components/RegionList";
import SalesMap from "./components/SalesMap";
import RegionDetailPanel from "./components/RegionDetailPanel";
import { SettingsProvider } from "./context/SettingsContext";
import SettingsPanel from "./components/SettingsPanel";
import { REGIONS } from "./utils/regionData";

function App() {
  const [selectedRegion, setSelectedRegion] = useState(null);
  const [isSettingsOpen, setIsSettingsOpen] = useState(false);

  const handleRegionSelect = (region) => {
    setSelectedRegion(region);
  };

  const handleRegionListSelect = (regionId) => {
    const region = REGIONS[regionId];
    if (region) {
      setSelectedRegion(region);
    }
  };

  return (
    <SettingsProvider>
      <div className="flex h-screen w-screen bg-gray-100 overflow-hidden relative">
        <RegionList onSelect={handleRegionListSelect} />
        <div className="flex-1 p-4 h-full relative">
          <SalesMap
            onRegionSelect={handleRegionSelect}
            selectedRegion={selectedRegion}
          />
          <RegionDetailPanel
            region={selectedRegion}
            onClose={() => setSelectedRegion(null)}
          />
        </div>

        {/* Settings Button */}
        <button
          onClick={() => setIsSettingsOpen(true)}
          className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-lg hover:bg-gray-50 transition-colors z-40"
          title="设置"
        >
          <svg className="w-6 h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
        </button>

        <SettingsPanel isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />
      </div>
    </SettingsProvider>
  );
}

export default App;
